...
class RNNPhonemeClassifier(object):
    ...
    def __init__(self, input_size, hidden_size, output_size, num_layers=2):
        ...
        self.rnn = [ RNNCell(input_size, hidden_size) if i == 0
    ...
    def forward(self, x, h_0=None):
        ...
        for t in range(seq_len):
            hidden = []
            # get the input for rnn cell
            input = self.x[:,t, :]
            # Iterate over the length of your self.rnn (through the layers)
            for i in range(len(self.rnn)):
                # Run the rnn cell with the correct parameters and update
                # the parameters as needed. Update hidden.
                # h_it: hidden state at at layer i time t
                h_it= self.rnn[i].forward(input, self.hiddens[-1][i]) #use i layer hidden state from previous timestamp
                input = h_it
                hidden.append(h_it)
            self.hiddens.append(hidden.copy()) # update hiddens
        logits = self.output_layer.forward(input) # feed in linear output layer
        ...
        * Normalize dh by batch_size since initial hidden states are also treated
        ...
        for t in range(seq_len -1, -1, -1):
            ...
            for i in range(self.num_layers -1, -1 , -1):
                # if i =0, input, else hidden state
                if i == 0:
                    h_prev_l = self.x[:,t, :]
                else:
                    h_prev_l = self.hiddens[t+1][i-1]
                ...
                dx, new_dh = self.rnn[i].backward(dh[i], self.hiddens[t+1][i], h_prev_l, h_prev_t)
                # Update dh with the new dh from the backward pass of the rnn cell
                dh[i] = new_dh
