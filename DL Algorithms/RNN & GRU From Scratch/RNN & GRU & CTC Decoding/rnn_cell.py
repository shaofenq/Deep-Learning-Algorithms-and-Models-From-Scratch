...
class RNNCell(object):
    ...
    def forward(self, x, h):
        ...
        """
        weighted_input = np.matmul(self.W_ih, x.T).T + self.b_ih
        weighted_hidden_state = np.matmul(self.W_hh, h.T).T + self.b_hh
        h_prime = self.activation.forward(weighted_input + weighted_hidden_state )
